...
1. 超越以往的物质基础：**我们的大脑（深度神经网络）**～～而这样的大脑已经足够精巧，能够解决识别物体这样复杂的问题
2. 再用一个**学习（训练）机制**：比如一对对输入（图像）、输出（分类）的数据，计算预测出分类，看分错了多少，**根据这些错误逐步修改大脑里的参数**，从而一步步减少分类错误

这样地把**特征提取**和**分类**放在一起来**训练**～～从而实现：

1. **自动**提取特征，不再需要人工设计特征啦
2. 自动根据这些特征来分类（识别物体），自己一气儿干下来
...
5. **归一化指数**（softmax）：把全连接分类器的输出值换算成每一个种类的概率
...
训练：用数据对中分错的，反向调整**深度神经网络（大脑）中的所有参数**～～别怕：**自动地**，所以叫机器学习嘛 : ) ～～一种优化算法，叫**反向传播 Back-propagation**

- 下山：逐渐减少错误
- 沿最陡的方向下山：
	+ w = w - gradient x learning rate
	+ 参数 = 参数 - 梯度（下山方向） x 学习率（每步迈多远）

### 卷积：提取特征

一个个**卷积核（小滤镜）**，每个滤镜扫一遍原始图或前面已经提取出来的**特征图**，生成（提取）一个新的特征图
